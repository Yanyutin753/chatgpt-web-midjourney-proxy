...
<template>
  <vue-turnstile
    ref="tRef"
    :site-key="homeStore.myData.session.turnstile"
    :appearance="appearance"
    v-model="homeStore.myData.vtoken"
    v-if="homeStore.myData.session.turnstile"
  />
      ...
      >
        ...
        >
          ...
          <SvgIcon
            ...
            @click="
              st.fileBase64.splice(st.fileBase64.indexOf(v), 1);
              st.fileName.splice(st.fileName.indexOf(v), 1);
            "
          ></SvgIcon>
        </div>
      </div>
